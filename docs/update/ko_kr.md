# 0.5.0 업데이트 내역
1. 이제, `builtins` 메소드를 호출할 때, 키워드 인자를 사용할 수 있습니다.
    ```
    a = foo(3, 4, _b=5, _c=6)
    ```
    * 즉, 이제 어떤 호출이든 키워드 인자를 사용할 수 있습니다.
2. `builtins`의 호출인자 변경으로 인해, 이전버전의 `builtins`와 호환되지 않습니다.
    * 다만, `mp`의 실사용에는 영향을 주지 않습니다.
3. 곧, IDE 를 더욱 강력하게 지원하기 위해, 코드 힌트 기능을 추가할 예정입니다.

### 0.4.2.6 업데이트 내역
1. 반복 호출에 관한 안정성을 강화하였습니다.

### 0.4.2.5 업데이트 내역
1. `mp.dataset.core`에서 데이터셋의 압축이 해제되지 않던 버그를 수정하였습니다.
2. 데이터셋 다운로드 상태바와 훈련 상태바가 겹치는 버그를 수정하였습니다.
